...
void motor_init()
{
    ...
    /* 配置定时器基本计数单元 */
    {
        ...
        temp.TIM_Period = 100-1;     /* 自动装载寄存器，[0, 65535]。这里100-1设置的是100个计数器周期作为溢出周期。
    ...
    TIM_ARRPreloadConfig(TIM3, DISABLE); //ENABLE-比较寄存器值修改后立即生效；DISABLE-比较寄存器值修改后下个周期生效
    /* 使能定时器 */
    TIM_Cmd(TIM3, ENABLE);
}
